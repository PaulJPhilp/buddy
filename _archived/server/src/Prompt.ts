...
export const PromptGroupLive = HttpApiBuilder.group(
  ...
  (handlers) =>
    handlers
      ...
      .handle("getPrompt", ({ path }) =>
        Effect.gen(function* () {
          const promptId = path.id;
          yield* log.info("Retrieving prompt", { id: promptId });

          const sql = yield* SqlClient;

          // Execute the select query directly with the SQL client
          const result = yield* logEffect.after(
            sql<Prompt>`SELECT id, name, type, prompt, tags, created_at, updated_at FROM prompts WHERE id=${promptId}`,
            "Retrieved prompt data from database",
          );

          if (result.length === 0) {
            yield* log.warn("Prompt not found", { id: promptId });
          ...
          const row = result[0];
          if (row === undefined) {
            yield* log.warn("Prompt result undefined", { id: promptId });
            return yield* Effect.fail(
              new NotFoundError({
                entityType: "Prompt",
                entityId: String(promptId),
          ...
          yield* log.info("Prompt found successfully", { id: promptId });
