import { type Prompt, PromptVoice, ServerApi } from "@api/core";
...
type PromptType = "user" | "system" | "template";

interface PromptVoiceQueryResult {
  id: string;
  name: string;
  type: string;
  label: string;
...
export const PromptVoiceGroupLive = HttpApiBuilder.group(
  ...
  (handlers) =>
    handlers
      ...
      .handle("getPromptVoice", ({ path }) =>
        Effect.gen(function* () {
          const promptVoiceId = path.id;
          yield* log.info("Retrieving prompt voice", { id: promptVoiceId });

          const sql = yield* SqlClient;

          const data = yield* Effect.tryPromise({
            try: async () => sql<PromptVoiceQueryResult>`
						SELECT 
							pv.id,
							pv.name,
							pv.type,
          ...
          if (result.length === 0 || result[0] === undefined) {
            yield* log.warn("Prompt voice not found", { id: promptVoiceId });
            return yield* Effect.fail(
              new NotFoundError({
                entityType: "PromptVoice",
                entityId: String(promptVoiceId),
                message: `PromptVoice with ID ${promptVoiceId} not found`,
              }),
            );
          }
