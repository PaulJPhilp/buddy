...
export const UIBar: React.FC<UIBarProps> = ({ elements, orientation = 'horizontal', className, gap = 'space-x-1' }) => {
    const layoutClasses = orientation === 'horizontal'
        ? `flex-row ${gap}`
        : `flex-col ${gap.replace('space-x-', 'space-y-')}`;

    return (
        <div className={`flex ${layoutClasses} items-center p-1 bg-muted/20 rounded-md border border-input ${className || ''}`}>
            {elements.map((element, index) => {
                const key = element.id || (element.type === "iconCommand" ? `icon-${element.iconName}-${index}` : `selector-${(element as SelectorElementConfig).placeholder || 'sel'}-${index}`);
                    ...
                    return (
                        ...
                        >
                            <span className="w-3.5 h-3.5 inline-flex items-center justify-center">
                                ...
                                {!["send", "emoji", "attach", "settings", "help"].includes(element.iconName) && (
                                    <span className="text-xs font-mono">{element.iconName.substring(0, 1).toUpperCase()}</span>
                                )}
                            </span>
                            {element.label && <span className="align-middle">{element.label}</span>}
                            {!element.label && <span className="sr-only">{element.iconName}</span>}
                        </button>
                    );
                }
                ...
                return <div key={unknownElement.id || `unknown-${JSON.stringify(unknownElement)}`} className="text-xs text-muted-foreground">Unknown UIBar element type</div>;
            })}
            {elements.length === 0 && <div className="text-xs text-muted-foreground p-1">UIBar (empty)</div>}
        </div>
    );
};
