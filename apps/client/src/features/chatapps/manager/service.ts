...
export class ChatAppsManager extends Effect.Service<ChatAppsManagerApi>()(
  "ChatAppsManager",
  {
    scoped: Effect.gen(function* () {
      ...
      const messageBus = yield* PubSub.unbounded<ChatAppBusMessage>();
      ...
      const handleCommand = (
        ...
        Effect.gen(function* () {
          switch (command._tag) {
            ...
            case "ExpandChatApp":
              console.log(
                `DEBUG: Processing ExpandChatApp command for ${command.appId}`
              );
              yield* expandChatApp(command.appId);
              console.log(
                `DEBUG: ExpandChatApp command completed for ${command.appId}`
              );
              break;
      ...
      const expandChatApp = (appId: string) =>
        Effect.gen(function* () {
          console.log(`DEBUG: expandChatApp called for ${appId}`);
          yield* setChatAppStatus(appId, "expanded");
          console.log(`DEBUG: expandChatApp completed for ${appId}`);
        });
      ...
      // Bus implementation
      const publishMessage = (message: ChatAppBusMessage) =>
        Effect.gen(function* () {
          const result = yield* PubSub.publish(messageBus, message);
          return result;
        }).pipe(
          Effect.mapError(
            (cause) =>
              new ChatAppsManagerOperationError({
                message: "Failed to publish message to bus",
      ...
      const subscribeToBus = () => Effect.succeed(messageBus);
