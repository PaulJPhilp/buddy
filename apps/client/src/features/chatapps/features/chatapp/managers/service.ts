...
function createMessage(command: SendMessage): MessageState {
  const now = new Date();
  return {
    id: command.messageId || `msg_${now.getTime()}`,
    conversationId: command.conversationId,
    role: command.sender === "assistant" ? "assistant" : "user",
...
export class ChatManager extends Effect.Service<ChatManagerApi>()(
  "ChatManager",
  {
    scoped: Effect.gen(function* () {
      ...
      // Subscribe to inter-app messages
      yield* Effect.forkDaemon(
        Stream.fromPubSub(messageBus).pipe(
          Stream.runForEach((busMessage) =>
            Effect.gen(function* () {
              ...
              const forwardCommand = new SendMessage({
                ...
                content: busMessage.message.content,
                sender: "user", // Always treat forwarded messages as user input
              });

              // Queue the command
              yield* commandQueue.offer(forwardCommand);
              ...
              yield* Ref.update(stateRef, (s) => {
                const conv = s.conversations[conversation.id];
                if (!conv) return s;
                return {
                  ...s,
                  conversations: {
              ...
              Effect.catchAll((error) =>
                Effect.sync(() =>
                  console.error("❌ Error in message bus handler:", error)
                )
              )
            )
          )
          ...
          case "SendMessage":
            return Effect.gen(function* () {
              ...
              yield* chatAppsManager
                .publishMessage({
                  sourceAppId: conversation.config?.id || conversation.id,
                  message: {
                    id: message.id,
                  ...
                  Effect.mapError(
                    (error) =>
                      new ChatManagerOperationError({
                        message: "Failed to publish message to chat apps",
                        operation: "SendMessage",
                        cause: error,
                      })
                  )
      ...
      return {
        ...
        startConversation: (agentId, initialMessage, config) =>
          Effect.gen(function* () {
