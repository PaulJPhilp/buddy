...
export function useChatInstance(
    ...
    const [dispatch, setDispatch] = useState<
        (action: ChatInstanceAction) => void
    >(
        () => () =>
    ...
    useEffect(() => {
        const program = Effect.gen(function* () {
            const agentRuntime = yield* ChatRuntimeService;
            yield* Effect.logInfo(
                `[useChatInstance] Program starting for ${chatId}, Agent: ${agentId}`,
            );
            ...
            const updateChatState = (
                updater: (prev: ChatInstanceHookState) => ChatInstanceHookState,
            ...
            const finalizeStreamingMessageInState = (streamId: string) =>
                Effect.gen(function* () {
            ...
            yield* Effect.logInfo(
                `[useChatInstance] Attempting to establish session for ${chatId}, agent ${agentId}`,
            );
                ...
                Effect.retry(
                    Schedule.intersect(
                        ...
                        Schedule.tapOutput((details) => {
                            const attempt =
                                typeof details === "number" ? details + 1 : details[1] + 1;
                            return updateChatState((prev) => ({
                                ...prev,
                                status: "reconnecting",
            ...
            yield* Effect.logInfo(
                `[useChatInstance] Session established: ${agentSession.id}, URL: ${agentSession.url}`,
            );

            yield* Effect.forkDaemon(
                agentSession.status$.pipe(
                            ...
                            switch (status._tag) {
                                ...
                                case "Connecting":
                                    setChatState((prev) => ({
                                        ...prev,
                                        status: "connecting",
                                        error: `Attempt ${status.attempt} to ${status.url}`,
                                    }));
                                    break;
                                case "Connected":
                                    setChatState((prev) => ({
                                ...
                                case "Disconnected":
                                    setChatState((prev) => ({
                                        ...prev,
                                        status: "disconnected",
                                        error: status.reason ?? "Connection closed",
                                    }));
                                    break;
                                case "Error":
                                    setChatState((prev) => ({
            ...
            yield* Effect.forkDaemon(
                Stream.fromQueue(inputQueue).pipe(
                    Stream.tap((action) =>
                        Effect.runFork(
                            Effect.logDebug("[useChatInstance] Action for outgoing", action),
                        ),
                    ),
                    Stream.filter(
                        (
                            action,
                        ): action is Extract<ChatInstanceAction, { _tag: "sendMessage" }> =>
                    ...
                    Stream.tap((message) =>
                        Effect.runFork(
                            Effect.logDebug(
                                `[useChatInstance] Sending protocol message for ${chatId}`,
                                message,
                            ),
                        ),
                    ),
                    Stream.runForEach((message) =>
                        agentSession.send(message).pipe(
                            Effect.catchAll((err) => {
                                Effect.logError(
                                    `[useChatInstance] Failed to send message for ${chatId}`,
                                    err,
                                );
                                return updateChatState((prev) => ({
                                    ...prev,
            ...
            yield* Effect.forkDaemon(
                agentSession.incomingMessages$.pipe(
                    Stream.tap((pm) =>
                        Effect.runFork(
                            Effect.logDebug(
                                `[useChatInstance] Raw incoming message for ${chatId}`,
                                pm,
                            ),
                        ),
                    ),
                    Stream.runForEach((protocolMessage: ProtocolMessage) =>
                        Effect.gen(function* () {
                            switch (protocolMessage.type) {
                                case "THINKING":
                                ...
                                case "LLM_STREAM":
                                    if (!protocolMessage.isComplete && protocolMessage.content) {
                                        const streamId =
                                            ...
                                            protocolMessage.id ||
                                            crypto.randomUUID();
                                        yield* updateStreamingMessageInState(
                                            streamId,
                                ...
                                default: {
                                    const uiMessage =
                                        yield* convertProtocolMessageToUIMessage(protocolMessage);
                            ...
                            Effect.catchAll((e) =>
                                Effect.logError("Error processing incoming message", e),
                            ),
                        ),
                    ),
                ),
            ...
            Effect.scoped,
            Effect.catchAll((error: AgentRuntimeError) =>
                Effect.gen(function* () {
                    yield* Effect.logError(
                        `[useChatInstance] Unrecoverable error in session management for ${chatId}`,
                        error,
                    );
        ...
        const fiber = Effect.runFork(
            program.pipe(Effect.provide(serviceLayer)) as Effect.Effect<
                void,
                never,
                never
            >,
        ...
        return () => {
            ...
            setDispatch(
                () => () =>
                    console.warn("Dispatch called after chat instance unmounted"),
            );
