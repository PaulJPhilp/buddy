...
function createTestLayer() {
  return Layer.merge(WebSocketService.Default, MdxService.Default);
}
...
describe("useChatInstance", () => {
  ...
  it("should handle connection gracefully with real services", async () => {
    ...
    expect(["error", "connecting", "disconnected", "initializing"]).toContain(
      result.current.chatState.status,
    );
  });
