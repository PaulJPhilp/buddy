...
export function useChatInstance(
  ...
  // Store subscriptions
  const chatInstanceState = useStore(
    chatInstanceStore,
    chatInstanceSelectors.getState,
  );
  const agentState = useStore(agentStore, agentSelectors.getState);
  const connectionState = useStore(
    connectionStore,
    connectionSelectors.getState,
  );
  ...
  useEffect(() => {
    const program = Effect.gen(function* () {
      yield* Effect.logInfo(
        `[useChatInstance] Initializing for chatId: ${chatId}, agentId: ${agentId}`,
      );
    ...
    const fiber = Effect.runFork(
      program.pipe(
        Effect.provide(Layer.provide(ChatInstanceBridge.Default, serviceLayer)),
        Effect.catchAll((error) =>
          Effect.gen(function* () {
            yield* Effect.logError(
              `[useChatInstance] Failed to initialize bridge for ${chatId}`,
              error,
            );
    ...
    return () => {
      Effect.runFork(
        Effect.gen(function* () {
          ...
          Effect.catchAll((error) =>
            Effect.logError("[useChatInstance] Error during cleanup", error),
          ),
        ),
      );
  ...
  const dispatchAction = useCallback(
    (action: ChatInstanceAction) => {
      if (!bridge) {
        console.warn(
          "[useChatInstance] Bridge not initialized, action ignored:",
          action,
        );
        return;
      }
      ...
      Effect.runFork(
        bridge
          ...
          .pipe(
            Effect.catchAll((error) =>
              Effect.logError(
                "[useChatInstance] Error processing action",
                error,
              ),
            ),
          ),
  ...
  }, [chatInstanceState, agentState.pendingMessages, chatId, initialAgentName]);
