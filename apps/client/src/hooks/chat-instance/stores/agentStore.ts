...
export const agentStore = createStore({
    ...
    on: {
        streamStarted: (context, event: { streamId: string; initialText?: string }) => {
        ...
        streamCompleted: (context, event: { streamId: string; finalMessage: Message }) => {
            const newActiveStreams = new Map(context.activeStreams);
...
export const agentSelectors = {
    ...
    // Get active streams
    getActiveStreams: (state: typeof agentStore.getSnapshot) => state().context.activeStreams,

    // Get pending messages
    getPendingMessages: (state: typeof agentStore.getSnapshot) => state().context.pendingMessages,

    // Check if a specific stream is active
    isStreamActive: (streamId: string) => (state: typeof agentStore.getSnapshot) =>
...
export const agentActions = {
    ...
    clearStreams: () =>
        agentStore.send({ type: 'streamsCleared' }),
};

// Utility functions for working with agent communication
export const agentUtils = {
    // Create a streaming message from current stream state
    createStreamingMessage: (streamId: string, role: 'user' | 'assistant' = 'assistant'): Message => {
        const currentText = agentSelectors.getStreamText(streamId)(agentStore.getSnapshot);
        return {
            id: streamId,
            text: currentText,
            role,
    ...
    finalizeStreamingMessage: (streamId: string, role: 'user' | 'assistant' = 'assistant'): Message => {
        const currentText = agentSelectors.getStreamText(streamId)(agentStore.getSnapshot);
        return {
            id: streamId,
            text: currentText,
