  ...
  return (
    <div className="h-screen flex flex-col max-w-2xl mx-auto p-4">
      ...
      <div className="mb-4 p-3 bg-gray-100 rounded text-sm">
        ...
        <p><strong>Messages Count:</strong> {chatState.messages.length}</p>
        <p><strong>Is Typing:</strong> {chatState.isTyping ? 'Yes' : 'No'}</p>
        {chatState.error && <p className="text-red-600"><strong>Error:</strong> {chatState.error}</p>}
        {runtimeError && <p className="text-red-600"><strong>Runtime Error:</strong> {runtimeError.message}</p>}
      </div>
      ...
      <div className="flex-1 border rounded p-4 mb-4 overflow-y-auto bg-white">
        <h3 className="font-semibold mb-2">Messages ({chatState.messages.length})</h3>
            ...
            {chatState.messages.map((message, index) => (
              ...
              >
                <div className="text-xs text-gray-600 mb-1">
                  {message.role} • {message.id} • {message.timestamp ? new Date(message.timestamp).toLocaleTimeString() : 'No timestamp'}
                </div>
