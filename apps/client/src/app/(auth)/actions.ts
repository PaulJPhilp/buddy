  ...
  try {
    ...
    try {
      ...
      const result = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}/api/user/create`,
        {
          method: "POST",
          headers: {
      ...
      if (
        ...
        console.error(
          "Connection error: The server may not be running on port 3001",
        );
      }
