...
export interface AgentConnectionApi {
    ...
    readonly receiveMessages: () => Stream.Stream<JsonObject, WebSocketMessageError, never>;
        ...
        effect: Effect.gen(function* (_) {
            ...
            const ensureConnection = Effect.gen(function* () {
                ...
                if (!ws || ws.readyState !== WebSocket.OPEN) {
                    return yield* Effect.fail(new WebSocketConnectionError({
            ...
            return {
                ...
                receiveMessages: () =>
                    Stream.async<JsonObject, WebSocketMessageError, never>(
                        (emit) => {
                            Effect.runSync(Effect.gen(function* () {
                                const ws = yield* Ref.get(connectionRef);
                                if (!ws) {
                                    emit(Effect.fail(Option.some(new WebSocketMessageError({
                                ...
                                ws.onmessage = (event) => {
                                    try {
                                        const parsedMessage: JsonObject = JSON.parse(event.data as string);
