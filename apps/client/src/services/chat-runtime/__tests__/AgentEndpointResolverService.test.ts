...
describe("AgentEndpointResolverService", () => {
  describe("resolveEndpoint", () => {
    it("should resolve endpoint for default agent", () =>
      Effect.gen(function* () {
        ...
        const endpoint = yield* service.resolveEndpoint("default-agent", "test-chat-123");
        
        expect(endpoint).toBe("ws://localhost:8080/chat?chatId=test-chat-123&agentId=default-agent");
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));

    it("should resolve endpoint for business agent", () =>
      Effect.gen(function* () {
        const service = yield* AgentEndpointResolverService;
        const endpoint = yield* service.resolveEndpoint("business-agent", "business-chat-456");
        
        expect(endpoint).toBe("ws://localhost:8080/chat?chatId=business-chat-456&agentId=business-agent");
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));

    it("should resolve endpoint for social agent", () =>
      Effect.gen(function* () {
        const service = yield* AgentEndpointResolverService;
        const endpoint = yield* service.resolveEndpoint("social-agent", "social-chat-789");
        
        expect(endpoint).toBe("ws://localhost:8080/chat?chatId=social-chat-789&agentId=social-agent");
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));

    it("should use default endpoint for unknown agent", () =>
      Effect.gen(function* () {
        const service = yield* AgentEndpointResolverService;
        const endpoint = yield* service.resolveEndpoint("unknown-agent", "unknown-chat-999");
        
        expect(endpoint).toBe("ws://localhost:8080/chat?chatId=unknown-chat-999&agentId=unknown-agent");
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should handle empty agent ID", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint).toBe("ws://localhost:8080/chat?chatId=empty-agent-chat&agentId=");
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should handle empty chat ID", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint).toBe("ws://localhost:8080/chat?chatId=&agentId=default-agent");
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));

    it("should handle special characters in chat ID", () =>
      Effect.gen(function* () {
        const service = yield* AgentEndpointResolverService;
        const endpoint = yield* service.resolveEndpoint("default-agent", "chat-with-special-chars!@#$%");
    ...
    it("should handle special characters in agent ID", () =>
      Effect.gen(function* () {
        const service = yield* AgentEndpointResolverService;
        const endpoint = yield* service.resolveEndpoint("agent-with-special-chars!@#", "test-chat");
        
        expect(endpoint.includes("chatId=test-chat").toBe(true));
        expect(endpoint.includes("agentId=agent-with-special-chars").toBe(true));
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should preserve existing path if it contains /chat", () =>
      Effect.gen(function* () {
        ...
        const service = yield* AgentEndpointResolverService;
        const endpoint = yield* service.resolveEndpoint("default-agent", "test-chat");
    ...
    it("should handle very long chat ID", () =>
      Effect.gen(function* () {
        ...
        const endpoint = yield* service.resolveEndpoint("default-agent", longChatId);
    ...
    it("should handle very long agent ID", () =>
      Effect.gen(function* () {
        ...
        const endpoint = yield* service.resolveEndpoint(longAgentId, "test-chat");
    ...
    it("should handle numeric chat ID", () =>
      Effect.gen(function* () {
        const service = yield* AgentEndpointResolverService;
        const endpoint = yield* service.resolveEndpoint("default-agent", "12345");
        
        expect(endpoint).toBe("ws://localhost:8080/chat?chatId=12345&agentId=default-agent");
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should handle numeric agent ID", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint).toBe("ws://localhost:8080/chat?chatId=test-chat&agentId=67890");
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should handle UUID-like IDs", () =>
      Effect.gen(function* () {
        ...
        const endpoint = yield* service.resolveEndpoint(uuidAgentId, uuidChatId);
    ...
    it("should handle multiple consecutive calls", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint1).toBe("ws://localhost:8080/chat?chatId=chat1&agentId=agent1");
        expect(endpoint2).toBe("ws://localhost:8080/chat?chatId=chat2&agentId=agent2");
        expect(endpoint3).toBe("ws://localhost:8080/chat?chatId=chat3&agentId=agent1");
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should be deterministic for same inputs", () =>
      Effect.gen(function* () {
        ...
        const endpoint1 = yield* service.resolveEndpoint("test-agent", "test-chat");
        const endpoint2 = yield* service.resolveEndpoint("test-agent", "test-chat");
  ...
  describe("URL construction edge cases", () => {
    it("should handle whitespace in IDs", () =>
      Effect.gen(function* () {
        const service = yield* AgentEndpointResolverService;
        const endpoint = yield* service.resolveEndpoint("agent with spaces", "chat with spaces");
