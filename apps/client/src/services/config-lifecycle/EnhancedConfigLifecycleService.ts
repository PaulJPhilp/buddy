...
// Enhanced state machine context with save status tracking
...
export interface EnhancedConfigLifecycleServiceApi {
  // Original methods
  ...
  readonly getState: () => Effect.Effect<EnhancedConfigLifecycleContext>;
  readonly subscribe: (
...
const createEnhancedConfigStore = () => {
  const initialContext: EnhancedConfigLifecycleContext = {
    configs: [],
    activeConfigId: null,
...
const loadConfigsFromFiles = Effect.gen(function* () {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const validatedConfig = yield* Schema.decodeUnknown(ChatAppConfigSchema)(
        configData,
...
export class EnhancedConfigLifecycleService extends Effect.Service<EnhancedConfigLifecycleServiceApi>()(
  "EnhancedConfigLifecycleService",
  {
    scoped: Effect.gen(function* () {
      const store = createEnhancedConfigStore();
...
// Default layer
export const EnhancedConfigLifecycleServiceLive =
