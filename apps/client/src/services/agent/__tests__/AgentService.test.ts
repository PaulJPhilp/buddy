...
describe("AgentService", () => {
  ...
  it("should return empty array initially", () =>
    Effect.gen(function* () {
      yield* Effect.logDebug("Starting test: should return empty array initially");
      
      yield* Effect.logDebug("Getting AgentService instance");
      const service = yield* AgentService;
      yield* Effect.logDebug("AgentService instance obtained", { service: typeof service });
      
      yield* Effect.logDebug("Calling service.getAll()");
      const agents = yield* service.getAll();
      yield* Effect.logDebug("service.getAll() completed", { agents, agentsLength: agents.length });
  ...
  it("should update an existing agent", () =>
    Effect.gen(function* () {
      ...
      yield* service.create(agent);
      yield* service.update("update-agent", { initialAgentName: "Updated Name" });
