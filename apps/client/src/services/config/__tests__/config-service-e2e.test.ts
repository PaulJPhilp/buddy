  ...
  describe("Performance and State Management", () => {
    ...
    it("should handle configuration health checks and repair", async () => {
      const program = Effect.gen(function* () {
        ...
        // Check health
        const health = yield* configService.checkConfigHealth(configNeedingRepair);
        expect(health.isHealthy).toBe(false);
        expect(health.issues.length).toBeGreaterThan(0);
        
        // Repair the config
        const repairedConfig = yield* configService.repairConfig(configNeedingRepair);
        expect(repairedConfig.app.name).toBe("Buddy");
        expect(repairedConfig.app.version).toBe("1.0.0");
        
        // Verify repair worked
        const repairedHealth = yield* configService.checkConfigHealth(repairedConfig);
    ...
    it("should handle configuration merging scenarios", async () => {
      const program = Effect.gen(function* () {
        ...
        baseConfig.workspaces.push(createDefaultWorkspaceConfig("Base Workspace"));
        
        // Create override config
        const overrideConfig = yield* configService.createDefaultConfig({
          app: { name: "Merged App" },
        });
        overrideConfig.workspaces.push(createDefaultWorkspaceConfig("Override Workspace"));
        
        // Test different merge strategies
        const mergedDefault = yield* configService.mergeConfigs(baseConfig, overrideConfig);
        expect(mergedDefault.app.name).toBe("Merged App");
        expect(mergedDefault.workspaces).toHaveLength(2);
        
        const mergedReplace = yield* configService.mergeConfigs(baseConfig, overrideConfig, {
          strategy: "replace",
        });
        expect(mergedReplace.workspaces).toHaveLength(1);
        expect(mergedReplace.workspaces[0].name).toBe("Override Workspace");
        
        const mergedAppend = yield* configService.mergeConfigs(baseConfig, overrideConfig, {
          strategy: "append",
        });
    ...
    it("should handle configuration validation with options", async () => {
      const program = Effect.gen(function* () {
        ...
        const validation = yield* configService.validateConfig(configWithDuplicates, {
    ...
    it("should handle configuration version detection and migration", async () => {
      const program = Effect.gen(function* () {
        ...
        const version1 = yield* configService.detectConfigVersion({ version: "2.0.0" });
        ...
        const migratedConfig = yield* configService.migrateConfig(oldConfig, "2.0.0");
