...
export interface AppServiceApi {
  ...
  create(
    app: ChatAppConfig,
  ): Effect.Effect<void, never, AgentService | ToolbarService>;
...
const validateReferences = (
  ...
  Effect.all([
    ...
    Effect.flatMap(
      themes.getTheme(app.themeId).pipe(
...
export class AppService extends Effect.Service<AppServiceApi>()(
  "AppService",
  {
    scoped: Effect.gen(function* () {
      const agents = yield* AgentService;
      const toolbars = yield* ToolbarService;
      const themes = yield* ThemesService;
      ...
      return {
        ...
        create: (app: ChatAppConfig) =>
          Effect.gen(function* () {
            ...
            yield* validateReferences(agents, toolbars, themes, validApp);
