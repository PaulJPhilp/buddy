...
export class AgentRuntimeService extends Effect.Service<AgentRuntimeServiceApi>()(
  "AgentRuntimeService", // Service Identifier
  {
    ...
    scoped: Effect.gen(function* () {
      ...
      function establishSessionImpl(
        agentId: string,
        chatId: string
      ): Effect.Effect<AgentSession, AgentRuntimeError, Scope> { // Scope from Effect.scoped
        return Effect.gen(function* () {
          ...
          yield* statusQueue.offer(AgentSessionStatus.Connecting(1, resolvedUrl));
          ...
          yield* wsService
            ...
            .pipe(
              Effect.mapError((err) =>
                mapToAgentRuntimeError(err, { agentId, chatId, url: resolvedUrl })
              )
            );
          ...
          const incomingMessages$ = wsService.receive().pipe(
            Stream.catchAll((error) => {
              ...
              return Stream.fromEffect(
                statusQueue.offer(AgentSessionStatus.Error(runtimeError, resolvedUrl))
          ...
          return {
            ...
            send,
            incomingMessages$,
            status$: Stream.fromQueue(statusQueue).pipe(Stream.ensuring(Queue.shutdown(statusQueue))),
            close: closeSession,
          } satisfies AgentSession;
        }).pipe(
          Effect.tapError((error) => { // Log failure of the entire establishSessionImpl attempt
      ...
      // Return the API implementation, conforming to AgentRuntimeServiceApi
      return {
