...
// Error types
export class SessionEstablishmentError extends Data.TaggedError(
...
export interface AgentCommunicationServiceApi {
  ...
  readonly createStatusStream: (
    ...
    AgentSession["status$"] extends Stream.Stream<infer T, any> ? T : never,
    never
  >;
}
    ...
    scoped: Effect.gen(function* () {
      ...
      const establishSession = (
        ...
        Effect.gen(function* () {
          ...
          const sessionEffect = chatRuntime
            .establishSession(agentId, chatId, prompt)
            .pipe(
              Effect.retry(
                Schedule.intersect(
      ...
      const sendMessage = (
        ...
        Effect.gen(function* () {
          ...
          console.log(
            "[AgentCommunicationService] About to send message:",
            protocolMessage,
          );
          ...
          yield* session.send(protocolMessage).pipe(
            Effect.mapError((error) => {
              console.error(
                "[AgentCommunicationService] Error in sendMessage:",
                error,
              );
          ...
          console.log(
            "[AgentCommunicationService] Message sent:",
            protocolMessage,
          );
        });
      ...
      const createIncomingMessageStream = (
        ...
        session.incomingMessages$.pipe(
          Stream.tap((message) => {
            console.log(
              "[AgentCommunicationService] Received message in stream:",
              message,
            );
            return Effect.succeed(undefined);
          }),
      ...
      > =>
        session.status$.pipe(
          Stream.tap((status) => {
            console.log(`[AgentCommunicationService] Status update`, status);
            return Effect.succeed(undefined);
          }),
        );
