import { ThemesService, type ThemeValidationError, type InvalidChatIdError, type ThemeNotFoundError, type ThemesServiceError, type ThemesServiceApi } from '../ThemesService';
...
describe('ThemesService (Effect.Service)', () => {
  ...
  const runTest = <R, E extends ThemesServiceError, A>(test: Effect.Effect<A, E, ThemesServiceApi>) => 
  ...
  it('validates theme objects', () => 
    runTest(Effect.gen(function*(_) {
      ...
      const result = yield* Effect.either(service.setTheme(chatId, invalidTheme));
  ...
  it('handles theme not found error', () => 
    runTest(Effect.gen(function*(_) {
      ...
      const result = yield* Effect.either(service.updateTheme('non-existent', { primary: '#ff0000' }));
      expect(Either.isLeft(result)).toBe(true);
      if (Either.isLeft(result)) {
