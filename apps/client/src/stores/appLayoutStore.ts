...
export const appLayoutStore = createStore({
    ...
    on: {
        ...
        setLayoutMode: (context, event: { mode: 'default' | 'compact' | 'wide' }) => ({
            ...context,
            layoutMode: event.mode,
        }),

        setScreenSize: (context, event: { size: 'sm' | 'md' | 'lg' | 'xl'; isMobile: boolean }) => ({
            ...context,
...
export function useSidebarState() {
    ...
    return {
        ...
        width: state.isSidebarOpen ? state.sidebarWidth : state.sidebarCollapsedWidth,
    }
} 