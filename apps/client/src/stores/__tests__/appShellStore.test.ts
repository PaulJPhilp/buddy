...
describe("appShellStore", () => {
    ...
    it("should create user and assistant messages with files", () => {
        ...
        const assistantMsg = useAppShellStore.getState().createAssistantMessage("hello", files)
    ...
    it("should manage attachments", () => {
        const attachment = { id: "1", name: "file.txt", size: 100, type: "text/plain" }
...
describe("chatStores (business/social)", () => {
    ...
    it("should add and create messages", () => {
        ...
        const msg = store.getState().createUserMessage("test", [
    ...
    it("should handle rating toolbar if present", () => {
        const store = useBusinessChatStore
        if (store.getState().hasRatingToolbar && typeof store.getState().rateMessage === "function") {
