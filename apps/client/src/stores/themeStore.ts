import { defaultChatTheme } from "@/features/chat/themes/themeTypes"
...
function generateCSSVariables(theme: ChatAppTheme): Record<string, string> {
    return {
        "--color-chat-background": theme.colors?.background || defaultChatTheme.colors?.background || "#ffffff",
        "--color-chat-foreground": theme.colors?.text || defaultChatTheme.colors?.text || "#000000",
        "--color-chat-primary": theme.colors?.primary || defaultChatTheme.colors?.primary || "#0066cc",
        "--color-chat-secondary": theme.colors?.secondary || defaultChatTheme.colors?.secondary || "#f5f5f5",
        "--color-chat-border": theme.borders?.color || defaultChatTheme.borders?.color || "#e0e0e0",
    }
}
