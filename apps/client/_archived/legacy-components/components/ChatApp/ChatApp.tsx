...
export function ChatApp({ config }: ChatAppProps) {
  ...
  const { messages, isTyping, status } = useChatState(chatService);
  ...
  useEffect(() => {
    ...
    // Use UrlService to resolve the WebSocket URL for this chat
    const program = Effect.gen(function* () {
      const configService = yield* UrlService;
      const wsUrl = yield* configService.buildChatUrl(config.id);
      return yield* chatService.initialize(config.id, wsUrl, config.agentId);
    });

    runWithServices(program).catch((error) => {
      console.error("[ChatApp] Initialization error:", error);
    });
  }, [chatService, runWithServices, config.id, config.agentId]);
  ...
  const handleSendMessage = useCallback(
    (text: string, files?: File[]) => {
      ...
      Effect.runPromise(chatService.sendMessage(text, files)).catch((error) => {
  ...
  const handleAgentChange = useCallback(
    (agentId: string) => {
      ...
      const program = chatService.switchAgent(agentId);
      runWithServices(program).catch((error) => {
  ...
  return (
    <ChatAppShell
      ...
      isTyping={isTyping}
      messages={messages}
      inputPlaceholder={config.inputPlaceholder ?? "Send a message..."}
