  ...
  describe("resolveEndpoint", () => {
    it("should resolve endpoint for default agent", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint).toBe(
          `${getExpectedWsBaseUrl()}?chatId=test-chat-123&agentId=default-agent`,
        );
    ...
    it("should resolve endpoint for business agent", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint).toBe(
          `${getExpectedWsBaseUrl()}?chatId=business-chat-456&agentId=business-agent`,
        );
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should resolve endpoint for social agent", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint).toBe(
          `${getExpectedWsBaseUrl()}?chatId=social-chat-789&agentId=social-agent`,
        );
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should use default endpoint for unknown agent", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint).toBe(
          `${getExpectedWsBaseUrl()}?chatId=unknown-chat-999&agentId=unknown-agent`,
        );
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should handle empty agent ID", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint).toBe(
          `${getExpectedWsBaseUrl()}?chatId=empty-agent-chat&agentId=`,
        );
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should handle empty chat ID", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint).toBe(
          `${getExpectedWsBaseUrl()}?chatId=&agentId=default-agent`,
        );
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should handle special characters in agent ID", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint.includes("agentId=agent-with-special-chars")).toBe(
          true,
        );
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should preserve existing path if it contains /chat", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint.startsWith(getExpectedWsBaseUrl())).toBe(true);
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should handle numeric chat ID", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint).toBe(
          `${getExpectedWsBaseUrl()}?chatId=12345&agentId=default-agent`,
        );
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should handle numeric agent ID", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint).toBe(
          `${getExpectedWsBaseUrl()}?chatId=test-chat&agentId=67890`,
        );
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
    ...
    it("should handle multiple consecutive calls", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint1).toBe(
          `${getExpectedWsBaseUrl()}?chatId=chat1&agentId=agent1`,
        );
        expect(endpoint2).toBe(
          `${getExpectedWsBaseUrl()}?chatId=chat2&agentId=agent2`,
        );
        expect(endpoint3).toBe(
          `${getExpectedWsBaseUrl()}?chatId=chat3&agentId=agent1`,
        );
      }).pipe(Effect.provide(AgentEndpointResolverService.Default)));
  ...
  describe("URL construction edge cases", () => {
    it("should handle whitespace in IDs", () =>
      Effect.gen(function* () {
        ...
        expect(endpoint.includes(getExpectedWsBaseUrl())).toBe(true);
...
// Helper to get the expected base WebSocket URL from env
const getExpectedWsBaseUrl = () => {
  const envUrl =
